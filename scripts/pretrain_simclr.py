    ...
    for _ in range(n_samples):
        ...
        for _ in range(n_signals):
            ...
            if sig_type == "drift":
                ...
                spec[:, t_start:t_end] += (
                    rng.standard_normal((freq_bins, t_end - t_start)).astype(np.float32)
                    * snr
                    * 0.02
                )
...
def main():
    ...
    parser.add_argument(
        "--temperature", type=float, default=0.07, help="NT-Xent temperature (default: 0.07)"
    )
    parser.add_argument(
        "--max-samples", type=int, default=5000, help="Max spectrograms to load (default: 5000)"
    )
    parser.add_argument(
        "--synthetic", action="store_true", help="Use synthetic data even if cache exists"
    )
    ...
    if not args.synthetic and CACHE_DIR.exists():
        spectrograms = load_cached_spectrograms(CACHE_DIR, max_samples=args.max_samples)
